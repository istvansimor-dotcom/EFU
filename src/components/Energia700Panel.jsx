/**
 * Energia700Panel.jsx — EFU 700.2 Közösségi Energia Szövetkezet (CEKS) v1.0
 *
 * Sections:
 *   1. Module header
 *   2. Változó kalibráció (sliders)
 *   3. CEKS index eredmény + pontszámok
 *   4. Energia & pénzügyi összefoglaló
...
import {
  ...
  CEKS_ZONES,
  CEKS_TRIGGERS,
} from '../data/energia700_2.js';
import { calculateCEKS } from '../logic/efu-engine.js';
    ...
    <div style={{ marginBottom: '12px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '3px' }}>
        <span style={{ fontSize: '12px', fontWeight: '700', color: variable.color }}>
          ...
          {!isPositive && (
            <span style={{ marginLeft: '5px', fontSize: '9px', background: '#dc2626', color: 'white', padding: '1px 5px', borderRadius: '3px' }}>−</span>
          )}
        </span>
        <span style={{ fontSize: '10px', color: '#6b7280', flex: 1, margin: '0 6px' }}>
          {variable.label}
        </span>
        <span style={{ fontSize: '12px', fontWeight: '700', color: variable.color, minWidth: '60px', textAlign: 'right' }}>
          {typeof value === 'number' && value % 1 === 0 ? value : value.toFixed(2)}{variable.unit ? ' ' + variable.unit : ''}
        </span>
      </div>
      <input
        type="range"
...
function CEKSGauge({ value, zone }) {
  const pct = Math.min((value / 10) * 100, 100);
  return (
    <div style={{ textAlign: 'center', padding: '16px 0' }}>
      <div style={{ fontSize: '12px', color: '#6b7280', marginBottom: '4px' }}>CEKS Index</div>
...
function ScoreBar({ label, score, color, weight }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px' }}>
      <span style={{ width: '110px', fontSize: '10px', fontWeight: '700', color, whiteSpace: 'nowrap' }}>{label}</span>
      <div style={{ flex: 1, height: '8px', background: '#e5e7eb', borderRadius: '4px', overflow: 'hidden' }}>
        <div style={{ width: `${pct}%`, height: '100%', background: color, borderRadius: '4px', transition: 'width 0.3s' }} />
      </div>
      <span style={{ width: '36px', fontSize: '10px', textAlign: 'right', color: '#374151', fontWeight: '700' }}>
        {score.toFixed(2)}
      </span>
      <span style={{ width: '30px', fontSize: '9px', textAlign: 'right', color: '#9ca3af' }}>
        ×{weight}
      </span>
    </div>
  );
      ...
      <div style={{ flex: 1 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ fontSize: '12px', fontWeight: '700', color: active ? trigger.color : '#374151' }}>
            {trigger.label}
            {isPositive && (
              <span style={{ marginLeft: '6px', fontSize: '9px', background: '#16a34a', color: 'white', padding: '1px 5px', borderRadius: '3px' }}>POZITÍV</span>
            )}
          </span>
...
export default function Energia700Panel() {
  const defaultVals = Object.fromEntries(CEKS_VARIABLES.map((v) => [v.id, v.default]));
  ...
  const result = useMemo(() => calculateCEKS(vals), [vals]);
  const { ceks_index, zone, triggers, energy, hmi, financial, sustainability, scores } = result;

  const jsonOutput = {
    module: MODULE_META_700_2.id,
    ceks_index,
    zone: zone.id,
    status: zone.status,
    series: MODULE_META_700_2.series,
    connections: MODULE_META_700_2.connections,
  ...
  return (
    <div style={{ fontFamily: 'system-ui, sans-serif', maxWidth: '860px', margin: '24px auto', padding: '0 16px' }}>
      ...
      {/* ── 1. Header ──────────────────────────────────────────────────────── */}
      <div style={{ background: 'linear-gradient(135deg, #14532d 0%, #166534 100%)', color: 'white', borderRadius: '10px', padding: '20px 24px', marginBottom: '20px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '10px' }}>
          <div>
            <div style={{ fontSize: '10px', letterSpacing: '0.1em', opacity: 0.7, textTransform: 'uppercase', marginBottom: '4px' }}>
              EFU 700 · Regeneratív Beavatkozások · Tier {MODULE_META_700_2.tier}
            </div>
            <div style={{ fontSize: '22px', fontWeight: '900', letterSpacing: '-0.02em' }}>
              700.2 Közösségi Energia Szövetkezet
            </div>
            <div style={{ fontSize: '12px', opacity: 0.8, marginTop: '4px' }}>
              {MODULE_META_700_2.titleEn}
            </div>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', alignItems: 'flex-end' }}>
            <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
              <Badge color="#166534">700 REGEN</Badge>
              <Badge color="#0369a1">CEKS</Badge>
              <Badge color="#374151">v1.0</Badge>
            </div>
            <div style={{ fontSize: '10px', opacity: 0.7, marginTop: '4px' }}>{MODULE_META_700_2.status}</div>
          </div>
        </div>
        <div style={{ marginTop: '10px', fontFamily: 'monospace', fontSize: '10px', opacity: 0.85, background: 'rgba(0,0,0,0.3)', borderRadius: '4px', padding: '6px 10px' }}>
          {MODULE_META_700_2.formula}
        </div>
        <div style={{ marginTop: '8px', fontSize: '10px', opacity: 0.7 }}>
          Connections: {MODULE_META_700_2.connections.join(' · ')}
        </div>
        <div style={{ marginTop: '8px', fontSize: '11px', opacity: 0.9, fontStyle: 'italic' }}>
          {MODULE_META_700_2.subtitle}
        </div>
        <div style={{ marginTop: '10px', display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
          <div style={{ padding: '6px 10px', background: 'rgba(255,255,255,0.15)', borderRadius: '6px', fontSize: '11px' }}>
            🎯 R_future cél: <strong>{MODULE_META_700_2.R_future_target}</strong>
          </div>
          <div style={{ padding: '6px 10px', background: 'rgba(255,255,255,0.15)', borderRadius: '6px', fontSize: '11px' }}>
            🧬 HMI cél: <strong>{MODULE_META_700_2.HMI_target}</strong>
          </div>
        </div>
      </div>

      {/* ── 2. Változó kalibráció ───────────────────────────────────────────── */}
      <SectionBox title="Változó Kalibráció (8 slider)" icon="🎛️" accentColor="#166534">
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 24px' }}>
          {CEKS_VARIABLES.map((variable) => (
      ...
      {/* ── 3. CEKS Index ───────────────────────────────────────────────────── */}
      <SectionBox title="CEKS – Közösségi Energia Szövetkezet Index" icon="📊" accentColor={zone.color}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', alignItems: 'start' }}>
          <CEKSGauge value={ceks_index} zone={zone} />
          <div>
            <div style={{ fontSize: '11px', fontWeight: '700', color: '#374151', marginBottom: '8px' }}>
              Komponens pontszámok:
            </div>
            <ScoreBar label="Energia" score={scores.energy_score} color="#d97706" weight="0.40" />
            <ScoreBar label="Governance" score={scores.governance_score} color="#16a34a" weight="0.30" />
            <ScoreBar label="Pénzügyi" score={scores.financial_score} color="#0369a1" weight="0.20" />
            <ScoreBar label="Fenntartható" score={scores.sustainability_score} color="#7c3aed" weight="0.10" />
            <div style={{ marginTop: '10px', paddingTop: '8px', borderTop: '1px solid #e5e7eb', fontSize: '10px', color: '#6b7280', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '3px' }}>
              <div>R_future: <strong style={{ color: '#166534' }}>{sustainability.R_future.toFixed(3)}</strong></div>
              <div>HMI total: <strong style={{ color: '#166534' }}>{hmi.HMI_total.toFixed(3)}</strong></div>
              <div>FLR total: <strong style={{ color: sustainability.flr_total > 25 ? '#dc2626' : '#374151' }}>{sustainability.flr_total.toFixed(1)}%</strong></div>
              <div>CO₂ elkerült: <strong>{sustainability.co2_t.toFixed(0)} t/év</strong></div>
            </div>
          </div>
        </div>
      </SectionBox>
      ...
      {/* ── 4. Energia & Pénzügyi összefoglaló ─────────────────────────────── */}
      <SectionBox title="Energia & Pénzügyi Összefoglaló" icon="💡" accentColor="#0369a1">
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
          {[
            { label: 'Bruttó termelés', value: `${energy.gross_gwh.toFixed(2)} GWh/év`, color: '#d97706' },
            { label: 'Felhasználható', value: `${energy.usable_gwh.toFixed(2)} GWh/év`, color: '#16a34a' },
            { label: 'Energia arány', value: energy.energy_ratio.toFixed(3), color: '#0369a1' },
            { label: 'Számlamegtakarítás', value: `${financial.bill_savings_k.toFixed(0)}k €/év`, color: '#16a34a' },
            { label: 'Export bevétel', value: `${financial.export_revenue_k.toFixed(0)}k €/év`, color: '#0369a1' },
            { label: 'Nettó profit', value: `${financial.profit_k.toFixed(0)}k €/év`, color: financial.profit_k >= 0 ? '#16a34a' : '#dc2626' },
          ].map((item, i) => (
            <div key={i} style={{ padding: '10px 14px', background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: '6px' }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
          <thead>
            <tr style={{ background: '#f3f4f6' }}>
              {['Zóna', 'CEKS érték', 'Státusz', 'Intézkedés'].map((h) => (
      ...
      {/* ── 6. Trigger logika ──────────────────────────────────────────────── */}
      <SectionBox title="Trigger Logika (4 trigger)" icon="⚡" accentColor="#166534">
        {CEKS_TRIGGERS.map((trigger) => (
