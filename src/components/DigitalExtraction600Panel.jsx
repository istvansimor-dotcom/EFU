/**
 * DigitalExtraction600Panel.jsx — EFU 600.53 Digitális Extrakció & AI Parazitizmus v0.3
 *
 * Sections:
 *   1. Modul fejléc (formula, cascade_modules, limitations)
 *   2. Változó kalibráció (extractive + protective sliders + params)
 *   3. CFI-D eredmény gauge
 *   4. Zóna táblázat
 *   5. Aktív triggerek
 *   6. Kaszkád diagram
 *   7. Beavatkozások
...
import {
  ...
  CFID_INTERVENTIONS,
  CFID_PLANETARY,
} from '../data/digitalExtraction600_53.js';
import {
  calculateCFID,
  classifyCFIDZone,
...
function VarSlider({ variable, value, onChange }) {
  ...
  const displayVal = variable.unit === 'h/nap' ? value.toFixed(1) : value.toFixed(2);
  return (
    <div style={{ marginBottom: '12px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '3px' }}>
        ...
        <span style={{ fontSize: '12px', fontWeight: '700', color: exceeded ? '#dc2626' : variable.color, minWidth: '60px', textAlign: 'right' }}>
          {displayVal}{variable.unit ? ` ${variable.unit}` : ''}{exceeded ? ' ⚠️' : ''}
        </span>
      </div>
      <input
        type="range"
...
export default function DigitalExtraction600Panel() {
  const initVars = () => {
    const v = {};
    for (const vr of CFID_VARIABLES) v[vr.id] = vr.default;
    return v;
  };
  const initParams = () => {
    const p = {};
    for (const [k, param] of Object.entries(CFID_PARAMS)) p[k] = param.default;
    return p;
  };
  ...
  const result = useMemo(
    () => calculateCFID({ ...varValues, ...paramValues }),
    [varValues, paramValues],
  );

  const { cfid_base, cfid_ai, cfid_net, cfid_total, zone, triggers, components, diagnostics } = result;
  const isFireChief = zone.id === 'RED' || zone.id === 'BLACK';

  const extractiveVars  = CFID_VARIABLES.filter(v => v.group === 'extractive');
  const protectiveVars  = CFID_VARIABLES.filter(v => v.group === 'protective');
  const paramList       = Object.values(CFID_PARAMS);

  const progressPct = Math.min(100, (cfid_total / 2000) * 100);

  return (
    <div style={{ fontFamily: 'system-ui, sans-serif', fontSize: '13px', color: '#111827', maxWidth: '900px', margin: '0 auto', padding: '20px' }}>

      {/* 1. Header */}
      <div style={{ background: 'linear-gradient(135deg, #0c4a6e 0%, #0369a1 100%)', color: 'white', borderRadius: '10px', padding: '20px', marginBottom: '20px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '10px' }}>
          <div>
            <div style={{ fontSize: '11px', opacity: 0.7, marginBottom: '4px' }}>
              EFU {MODULE_META_53.id} · {MODULE_META_53.series}
            </div>
            <h2 style={{ margin: 0, fontSize: '20px', fontWeight: '800' }}>{MODULE_META_53.title}</h2>
            <div style={{ fontSize: '13px', opacity: 0.85, marginTop: '4px' }}>{MODULE_META_53.subtitle}</div>
          </div>
          <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
            <Badge color="#dc2626">{MODULE_META_53.mechanism_primary}</Badge>
            <Badge color="#7c3aed">{MODULE_META_53.mechanism_secondary}</Badge>
            <Badge color="#374151">W_irrev = {MODULE_META_53.W_irrev}</Badge>
            <Badge color="#374151">{MODULE_META_53.status}</Badge>
          </div>
        </div>
        <div style={{ marginTop: '12px', background: 'rgba(0,0,0,0.25)', borderRadius: '6px', padding: '10px', fontSize: '10px', fontFamily: 'monospace' }}>
          {MODULE_META_53.formula.split('\n').map((line, i) => (
            <div key={i} style={{ color: '#fbbf24', marginBottom: '2px' }}>{line}</div>
          ))}
        </div>
        <div style={{ marginTop: '8px', display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
          {MODULE_META_53.cascade_modules.map(m => (
            <span key={m} style={{ fontSize: '9px', background: 'rgba(255,255,255,0.15)', padding: '2px 7px', borderRadius: '10px' }}>⇄ {m}</span>
          ))}
        </div>
        {MODULE_META_53.limitations && (
          <div style={{ marginTop: '10px', fontSize: '9px', opacity: 0.65 }}>
            ⚠️ Limitációk: {MODULE_META_53.limitations.join(' · ')}
          </div>
        )}
      </div>

      {/* 2. Calibration */}
      <SectionBox title="Változó Kalibráció – CFI-D" icon="⚙️" accentColor="#d1d5db">
        <div style={{ fontSize: '11px', fontWeight: '700', color: '#0369a1', marginBottom: '8px', textTransform: 'uppercase' }}>
          📉 Extraktív változók
        </div>
      ...
      {/* 3. Result gauge */}
      <SectionBox title="CFI-D Eredmény" icon="📊" accentColor={zone.color}>
        <div style={{ display: 'flex', gap: '20px', alignItems: 'flex-start', flexWrap: 'wrap' }}>
          <div style={{ textAlign: 'center', background: zone.color, color: 'white', borderRadius: '10px', padding: '16px 24px', minWidth: '130px' }}>
            <div style={{ fontSize: '11px', opacity: 0.85, marginBottom: '4px' }}>CFI-D TOTAL</div>
            <div style={{ fontSize: '32px', fontWeight: '900', lineHeight: 1 }}>{cfid_total.toFixed(1)}</div>
            <div style={{ fontSize: '12px', marginTop: '6px', opacity: 0.9 }}>{zone.label || zone.id}</div>
          ...
          <div style={{ flex: 1, minWidth: '200px' }}>
            ...
            <div style={{ fontSize: '11px', color: '#6b7280', lineHeight: '1.8' }}>
              <div>CFI-D base: <strong>{cfid_base.toFixed(1)}</strong></div>
              <div>CFI-D + AI erősítés: <strong>{cfid_ai.toFixed(1)}</strong></div>
              <div>CFI-D net (−regen): <strong>{cfid_net.toFixed(1)}</strong></div>
              <div style={{ borderTop: '1px solid #e5e7eb', paddingTop: '4px', marginTop: '4px' }}>
                F_detox: <strong>{components.F_detox.toFixed(3)}</strong> · B_addikció: <strong>{components.B_addikcio.toFixed(3)}</strong> · raw: <strong>{components.raw.toFixed(3)}</strong>
              </div>
        ...
        {isFireChief && (
          <div style={{ marginTop: '12px', background: '#111827', color: '#fbbf24', padding: '10px 14px', borderRadius: '6px', fontSize: '11px', fontWeight: '700' }}>
            🔴 FIRE CHIEF SZINT AKTÍV — Szuverén digitális infrastruktúra program azonnali indítás szükséges.
          </div>
        )}
      </SectionBox>

      {/* 4. Zone table */}
      <SectionBox title="Zóna Táblázat" icon="🌐" accentColor="#d1d5db">
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '11px' }}>
          <thead>
            <tr style={{ background: '#f3f4f6' }}>
              {['Zóna', 'CFI-D', 'Státusz', 'Dashboard akció', 'Allokáció'].map(h => (
      ...
      {/* 8. Planetary scale */}
      <SectionBox title="Planetáris Skála" icon="🌍" accentColor="#bfdbfe">
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
          {CFID_PLANETARY.map((p, i) => (
            <div key={i} style={{ background: '#f0f9ff', border: `1px solid ${p.color}`, borderRadius: '6px', padding: '10px 14px' }}>
